...
def draw_agent(
    ...
    **kwargs,
    ...
    if np.isnan(agent_extent).any():
        ...
        # print(agent_extent)
        length = agent_extent[0]
        width = agent_extent[1]

    # xy = agent_state.position
    # heading = agent_state.heading

    xy = agent_state[:2]
    heading = agent_state[3:]

    patch = FancyBboxPatch([-length / 2, -width / 2], length, width, **kwargs)
    transform = (
        mtransforms.Affine2D().rotate(heading[0]).translate(xy[0], xy[1])
...
def plot_scene_batch(
    ...
    agent_from_world_tf: Tensor = batch['centered_agent_state']
    ...
    agent_extent = batch['agent_hist_extent'][:, -1]
    ...
    for agent_id in range(num_agents):
        ...
        #     continue
        np.isnan(agent_hist[agent_id, -1]).any()
    ...
    if 'robot_fut' in batch and batch['robot_fut'].shape[1] > 0:
        ax.plot(
...
def plot_full_map(batch, lim_scale, controlled_names, ax=None, show_gt=False, agent_name_to_color={}):
  ...
  # vec_map = batch['vec_map']
  # center_in_world_xyzh = batch['centered_agent_state']
  # vecs = extract_lane_vecs(vec_map, center_in_world_xyzh, 150)
  # polyline_tensors_local = batch['extras.road_edge_polyline']
  vecs = batch['vis_vecs']
  center = vecs['center']
  ...
  #   ax.plot(lane[:, 0], lane[:, 1], color='gray', linewidth=1.5, alpha=0.8)
  
  return ax, agent_name_to_color

def plot_batch_prompts(batch, ax, agent_name_to_color, controlled_names, font_size=10.0, show_all_name=False, all_text_pos={}, plot_goal_text=True):
  ...
  if 'position_t' in batch['extras.prompt']['motion_pred'].keys():
    all_init_pos_t = batch['extras.prompt']['motion_pred']['position_t'].cpu().numpy()
    all_init_heading_t = batch['extras.prompt']['motion_pred']['heading_t'].cpu().numpy()
  else:
    all_init_pos_t = all_init_pos
    all_init_heading_t = all_init_heading

  # print(all_init_pos_t)
  ...
  for nidx, name in enumerate(batch['extras.prompt']['motion_pred']['agent_ids'][0]):
    ...
    if is_prompted:
      ...
      pos_t = all_init_pos_t[0, nidx, :2]
      heading_t = all_init_heading_t[0, nidx]
      ...
      if plot_prompt:
        if text_pos[0] == -1:
            pass
        if text_pos[0] == 0: # up
            ax.text(pos_t[0], pos_t[1]-1.5, agent_name, fontsize=font_size, color=color, ha='center', va='bottom', fontweight=800, clip_on=True)
        elif text_pos[0] == 1: # down
            if nidx == 8:
              ax.text(pos_t[0], pos_t[1]+1.0, agent_name, fontsize=font_size, color=color, ha='center', va='top', fontweight=800, clip_on=True)
            else:
              ax.text(pos_t[0], pos_t[1]+1.5, agent_name, fontsize=font_size, color=color, ha='center', va='top', fontweight=800, clip_on=True)
        elif text_pos[0] == 2: # right
            if nidx == 23:
              ax.text(pos_t[0]+3.0, pos_t[1]-1.5, agent_name, fontsize=font_size, color=color, ha='left', va='center', fontweight=800, clip_on=True)
            else:
              ax.text(pos_t[0]+3.0, pos_t[1], agent_name, fontsize=font_size, color=color, ha='left', va='center', fontweight=800, clip_on=True)
        elif text_pos[0] == 3: # left
            ax.text(pos_t[0]-3.0, pos_t[1], agent_name, fontsize=font_size, color=color, ha='right', va='center', fontweight=800, clip_on=True)
      else:
        ax.text(pos_t[0], pos_t[1]-1.5, agent_name, fontsize=8.0, color='r', ha='center', va='bottom', fontweight=300, clip_on=True)
      ...
      if 'goal' in batch['extras.condition'].keys() and plot_prompt:
        ...
        if goal_cond_mask[cidx]:
          ...
          if text_pos[1] == -1 or plot_goal_text == False:
            pass
...
def plot_demo_fig(batch, controlled_names, output, scale=0.85, hide_axis=True, save_path=None, show_all_name=False, show_gt=False, all_text_pos={}, agent_name_to_color={}):
    fig, ax = plt.subplots()

    ax, agent_name_to_color = plot_full_map(batch, scale, controlled_names, ax, show_gt, agent_name_to_color)
