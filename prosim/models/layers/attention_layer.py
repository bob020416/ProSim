# Mostly copied from https://github.com/ZikangZhou/QCNet/blob/main/layers/attention_layer.py
...
from prosim.models.utils import weight_init
...
class AttentionLayer(MessagePassing):
    ...
    def forward(self,
        ...
        if isinstance(x, torch.Tensor):
            x_src = self.attn_prenorm_x_src(x)
            x_dst = self.attn_prenorm_x_dst(x)
        else:
            x_src, x_dst = x
        ...
        x_src = x_src.to(x.dtype)
        x_dst = x_dst.to(x.dtype)
        r = r.to(x.dtype)

        # cast postnorm to the same dtype as q_i (needed for torch.bfloat16)
        x = x + self.attn_postnorm(self._attn_block(x_src, x_dst, r, edge_index)).to(x.dtype)
        x = x + self.ff_postnorm(self._ff_block(self.ff_prenorm(x))).to(x.dtype)
        return x
    ...
    def message(self,
        ...
        # cast attn to the same dtype as q_i (needed for torch.bfloat16)
        # start = time.time()
        attn = attn.to(q_i.dtype)
        # cast_time = time.time() - start
        # print('\t\t\tAttn message dtype cast_time:', cast_time)
