...
def rollout_baseline(waymo_scene):
  logged_trajectories, simulated_states = simulate_with_extrapolation(
    waymo_scene, print_verbose_comments=False)

  scenario_rollouts = scenario_rollouts_from_states(
      waymo_scene, simulated_states, logged_trajectories.object_id.numpy())
  # As before, we can validate the message we just generate.
  submission_specs.validate_scenario_rollouts(scenario_rollouts, waymo_scene)

  return scenario_rollouts

def get_waymo_scene_object(file_path):
